...
interrupt 20 void Rx_SetDriver(void)
{
    ...
    (void)sprintf(SCIreceive,"当前摆头舵机值为:%d",temp_pwm67);  
    ...
    (void)sprintf(SCIreceive,"当前摆头舵机值为:%d",temp_pwm67);  
     SCISend_chars(SCIreceive); 
      break;
        ...
        temp_pwm67=PWM67; 
		Light_SetDriver(temp_pwm67);
	(void)sprintf(SCIreceive,"摆头舵机复位，当前舵机值为:%d",temp_pwm67);  
     SCISend_chars(SCIreceive); 
      break;
            case 'o':                          //转向摆头舵机向左偏参数 5度档
        temp_pwm45=temp_pwm45+5;
        SCI_SetDriver(temp_pwm45);
		(void)sprintf(SCIreceive,"当前转角舵机值为:%d",temp_pwm45);  
     SCISend_chars(SCIreceive); 
      break;
     ...
     SCISend_chars(SCIreceive); 
      break;
        ...
        temp_pwm45=PWM45; 
        SCI_SetDriver(temp_pwm45);
		(void)sprintf(SCIreceive,"转角舵机复位，当前舵机值为:%d",temp_pwm45);  
     SCISend_chars(SCIreceive); 
      break;
    }
    EnableInterrupts;
